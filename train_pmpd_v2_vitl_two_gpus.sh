...
LOG_DIR="$REPO_DIR/training_logs/pmpd_v2_vitl_bce_cv"
OUTPUT_DIR="outputs/pmpd_v2_vitl_bce_cv"
RUN_NAME="anymc3d-dinov3-vitl-pmpd-v2-bce-balanced"
...
echo "$(date -Is) Starting ViT-L GPU 0 folds [1,3,5]"
run_fold_group 0 '[1,3,5]' "$LOG_DIR/gpu0_folds_1_3_5.log" &
PID_GPU0=$!

echo "$(date -Is) Starting ViT-L GPU 1 folds [2,4]"
...
echo "$(date -Is) Aggregating ViT-L folds 1-5"
"$PYTHON" -c \
    'from pathlib import Path; from train_cv import aggregate; aggregate([1, 2, 3, 4, 5], Path("outputs/pmpd_v2_vitl_bce_cv"))'
echo "$(date -Is) PMPD-v2 ViT-L five-fold training completed"
